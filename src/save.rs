...
pub fn read_save(gba: &mut agb::Gba, length: usize) -> Result<Vec<bool, InternalAllocator>, Error> {
    let mut access = gba.save.access()?;

    let mut bytes: Vec<u8, InternalAllocator> = Vec::with_capacity_in(30, InternalAllocator);
    ...
    let mut booleans: Vec<bool, InternalAllocator> =
        Vec::with_capacity_in(bytes.len(), InternalAllocator);
